...
function AssistantWizard() {
  ...
  useEffect(() => {
    ...
    if (
      stepFromState &&
      Object.values(ASSISTANT_STEPS).includes(stepFromState)
    ) {
  ...
  const handleContinue = () => {
    ...
    if (currentStep === ASSISTANT_STEPS.RECIPIENT_PHONE && recipientPhone) {
      ...
      currentStep === ASSISTANT_STEPS.PICKUP_ADDRESS &&
      pickupAddress &&
      pickupSenderName
    ) {
      nextStep = ASSISTANT_STEPS.CONTACT_PHONE;
    } else if (
      currentStep === ASSISTANT_STEPS.CONTACT_PHONE &&
      contactPhone &&
      !auth.codeSent
    ) {
      nextStep = "send_code";
  ...
  const renderStep = () => {
    switch (currentStep) {
      ...
      case ASSISTANT_STEPS.ORDER_COMPLETE: {
        ...
        return <OrderCompleteStep wizardData={wizardDataForComplete} />;
      }
  ...
  return (
    <WizardLayout
      ...
      progress={
        currentStep === ASSISTANT_STEPS.ORDER_COMPLETE
          ? 100
          : currentStep === ASSISTANT_STEPS.CONTACT_PHONE
            ? auth.codeSent
              ? 75
