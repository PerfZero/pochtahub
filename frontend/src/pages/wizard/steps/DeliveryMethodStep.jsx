...
function DeliveryMethodStep({
  ...
  const isDisabled =
    !deliveryMethod || (deliveryMethod === "courier" && !isAddressValid);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 mb-6 md:mb-8">
        ...
        >
          <span className="flex flex-col items-center gap-3 md:gap-4">
            ...
            <span className="text-sm md:text-base font-semibold text-[#2D2D2D] text-center">
      ...
      {deliveryMethod === "pickup" && (
        <div className="mb-6 p-3 md:p-4 bg-[#F5F5F5] rounded-xl flex items-start gap-2 md:gap-3">
          ...
          <p className="text-xs md:text-sm text-[#2D2D2D]">
            Мы покажем ближайшие пункты приёма без очереди после подтверждения
            отправки.Нажмите "Продолжить".
          </p>
        </div>
      )}
      <button
