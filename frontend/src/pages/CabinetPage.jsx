...
function CabinetPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      console.log('Загруженные заказы:', ordersList.map(o => ({
        id: o.id,
      ...
      setFullName([profileRes.data.first_name, profileRes.data.last_name].filter(Boolean).join(' ') || '')
        ...
        <div className="w-full max-w-[720px] bg-white rounded-2xl p-4 md:p-8 mx-4 md:mx-6">
          ...
          <div className="flex gap-2 mb-6 md:mb-8 justify-center">
            <button
              ...
              className={`px-6 py-3 rounded-xl text-sm font-semibold transition-colors ${
                activeTab === 'profile' ? 'bg-[#0077FE] text-white' : 'bg-[#F5F5F5] text-[#2D2D2D]'
              }`}
            >
              Мой профиль
            </button>
            <button
              ...
              className={`px-6 py-3 rounded-xl text-sm font-semibold transition-colors ${
                activeTab === 'orders' ? 'bg-[#0077FE] text-white' : 'bg-[#F5F5F5] text-[#2D2D2D]'
              }`}
            >
              Мои доставки
            </button>
          ...
          {activeTab === 'profile' && (
            <div className="space-y-6 md:space-y-8">
              <div>
                <h2 className="text-lg md:text-xl font-bold text-[#2D2D2D] mb-4 md:mb-6">Мой профиль</h2>
                <div className="space-y-4 md:space-y-6">
                  <div>
                    <label className="block text-sm text-[#858585] mb-2">ФИО</label>
                    <input
                      type="text"
                      value={fullName}
                  ...
                  <div>
                    <label className="block text-sm text-[#858585] mb-2">Email</label>
                    <input
                      type="email"
                      value={email}
                      disabled
              ...
              <div>
                <h2 className="text-lg md:text-xl font-bold text-[#2D2D2D] mb-4 md:mb-6">Адрес</h2>
                <div>
                  <label className="block text-sm text-[#858585] mb-2">Адрес</label>
                  <div className="relative">
                    <div className="absolute left-4 top-1/2 -translate-y-1/2">
                      <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M10 10C11.3807 10 12.5 8.88071 12.5 7.5C12.5 6.11929 11.3807 5 10 5C8.61929 5 7.5 6.11929 7.5 7.5C7.5 8.88071 8.61929 10 10 10Z" stroke="#858585" strokeWidth="1.5"/>
                        <path d="M10 17.5C13.3333 14.1667 16.6667 11.5152 16.6667 7.5C16.6667 4.46243 14.2043 2 10 2C5.79566 2 3.33334 4.46243 3.33334 7.5C3.33334 11.5152 6.66667 14.1667 10 17.5Z" stroke="#858585" strokeWidth="1.5"/>
                      </svg>
                    </div>
                    <input
                      type="text"
              ...
              {orders.length === 0 ? (
                ...
                orders.map((order) => {
                  ...
                  const getDeliveryDays = () => {
                    if (order.tariff_name && order.tariff_name.includes('2-3')) return '2-3 дн.'
                    if (order.tariff_name && order.tariff_name.includes('3-5')) return '3-5 дн.'
                    if (order.tariff_name && order.tariff_name.includes('1-2')) return '1-2 дн.'
                    return '2-3 дн.'
                  }

                  const orderNumber = order.external_order_number || `#${order.id}`
                  ...
                  const totalPrice = order.total_price || (deliveryCost + packagingCost + insuranceCost + commission + acquiringCost)

                  const logoUrl = order.transport_company_logo ? getMediaUrl(order.transport_company_logo) : null
                  if (logoUrl) {
                    console.log(`Заказ ${order.id}: логотип = ${order.transport_company_logo}, обработанный URL = ${logoUrl}`)
                  } else {
                    console.log(`Заказ ${order.id}: transport_company_logo = ${order.transport_company_logo}, transport_company_id = ${order.transport_company_id}`)
                  }

                  return (
                    <div key={order.id} className="bg-white border border-[#C8C7CC] rounded-2xl overflow-hidden">
                        ...
                        >
                          {logoUrl ? (
                            <img 
                              ...
                              onError={(e) => {
                                console.error('Ошибка загрузки логотипа:', logoUrl, 'Исходный путь:', order.transport_company_logo)
                            ...
                            <div className="w-10 h-10 rounded-full bg-[#0C3739] flex items-center justify-center shrink-0 border border-[#ECE9EB]">
                              <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M10 2L2 6L10 10L18 6L10 2Z" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                                <path d="M2 14L10 18L18 14" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                                <path d="M2 10L10 14L18 10" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                              </svg>
                            </div>
                          )}
                          ...
                          <div className="shrink-0">
                            ...
                            >
                              <path d="M6 9L12 15L18 9" stroke="#2D2D2D" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                            </svg>
                          </div>
                        </button>
                      </div>
                      ...
                      {isExpanded && (
                        <div className="px-4 py-3">
                          <div className="bg-[#F4F2F3] rounded-xl overflow-hidden">
                            <div className="border-b border-[#C8C7CC] border-dashed">
                              <div className="px-4 py-3 flex justify-between items-center">
                                <span className="text-sm text-[#2D2D2D]">Транспортная компания</span>
                                <span className="text-sm text-[#2D2D2D]">{order.transport_company_name || '-'}</span>
                              </div>
                            </div>
                            <div className="border-b border-[#C8C7CC] border-dashed">
                              <div className="px-4 py-3 flex justify-between items-center">
                                <span className="text-sm text-[#2D2D2D]">Откуда</span>
                            ...
                            <div className="border-b border-[#C8C7CC] border-dashed">
                              <div className="px-4 py-3 flex justify-between items-center">
                                <span className="text-sm text-[#2D2D2D]">Куда</span>
                                <span className="text-sm text-[#2D2D2D] text-right">
                            ...
                            <div className="border-b border-[#C8C7CC] border-dashed">
                              <div className="px-4 py-3 flex justify-between items-center">
                                <span className="text-sm text-[#2D2D2D]">Телефон отправителя</span>
                                <span className="text-sm text-[#2D2D2D]">{order.sender_phone || '-'}</span>
                              </div>
                            </div>
                            <div className="border-b border-[#C8C7CC] border-dashed">
                              <div className="px-4 py-3 flex justify-between items-center">
                                <span className="text-sm text-[#2D2D2D]">Телефон получателя</span>
                                <span className="text-sm text-[#2D2D2D]">{order.recipient_phone || '-'}</span>
                              </div>
                            </div>
                            <div className="border-b border-[#C8C7CC] border-dashed">
                              <div className="px-4 py-3 flex justify-between items-center">
                                <span className="text-sm text-[#2D2D2D]">Кто оплачивает</span>
                                <span className="text-sm text-[#2D2D2D]">
                                  {order.selected_role === 'recipient' ? 'Получатель' : 'Отправитель'}
                                </span>
                              </div>
                            </div>
                            <div className="border-b border-[#C8C7CC] border-dashed">
                              <div className="px-4 py-3 flex justify-between items-center">
                                <span className="text-sm text-[#2D2D2D]">Габариты посылки</span>
                                <span className="text-sm text-[#2D2D2D] text-right">
                            ...
                            <div className="border-b border-[#C8C7CC] border-dashed">
                              <div className="px-4 py-3 flex justify-between items-center">
                                <span className="text-sm text-[#2D2D2D]">Стоимость</span>
                                <span className="text-sm text-[#2D2D2D]">{order.price}₽</span>
                              </div>
                            </div>
                            <div className="border-b border-[#C8C7CC] border-dashed">
                              <div className="px-4 py-3 flex justify-between items-center">
                                <span className="text-sm text-[#2D2D2D]">Дата отправления</span>
                                <span className="text-sm text-[#2D2D2D]">{formatDateTime(order.created_at)}</span>
                              </div>
                            </div>
                            <div className="border-b border-[#C8C7CC] border-dashed">
                              <div className="px-4 py-3 flex justify-between items-center">
                                <span className="text-sm text-[#2D2D2D]">Дата получения</span>
                                <span className="text-sm text-[#2D2D2D]">
                            ...
                            {packagingCost > 0 && (
                              <div className="border-b border-[#C8C7CC] border-dashed">
                                <div className="px-4 py-3 flex justify-between items-center">
                                  <span className="text-sm text-[#2D2D2D]">Стоимость упаковки</span>
                                  <span className="text-sm text-[#2D2D2D]">{packagingCost.toLocaleString('ru-RU')}₽</span>
                                </div>
                              </div>
                            )}
                            {insuranceCost > 0 && (
                              <div className="border-b border-[#C8C7CC] border-dashed">
                                <div className="px-4 py-3 flex justify-between items-center">
                                  <span className="text-sm text-[#2D2D2D]">Страховка</span>
                                  <span className="text-sm text-[#2D2D2D]">{insuranceCost.toLocaleString('ru-RU')}₽</span>
                                </div>
                              </div>
                            )}
                            {commission > 0 && (
                              <div className="border-b border-[#C8C7CC] border-dashed">
                                <div className="px-4 py-3 flex justify-between items-center">
                                  <span className="text-sm text-[#2D2D2D]">Комиссия PochtaHub</span>
                                  <span className="text-sm text-[#2D2D2D]">{commission.toLocaleString('ru-RU')}₽</span>
                                </div>
                              </div>
                            )}
                            {acquiringCost > 0 && (
                              <div className="border-b border-[#C8C7CC] border-dashed">
                                <div className="px-4 py-3 flex justify-between items-center">
                                  <span className="text-sm text-[#2D2D2D]">Эквайринг</span>
                                  <span className="text-sm text-[#2D2D2D]">{acquiringCost.toLocaleString('ru-RU', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}₽</span>
                                </div>
                              </div>
                            )}
                            <div className="border-b border-[#C8C7CC] border-dashed">
                              <div className="px-4 py-3 flex justify-between items-center">
                                <span className="text-sm text-[#2D2D2D]">Стоимость доставки</span>
                                <span className="text-sm text-[#2D2D2D]">{deliveryCost.toLocaleString('ru-RU', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}₽</span>
                              </div>
                            </div>
                            <div>
                              <div className="px-4 py-3 flex justify-between items-center">
                                <span className="text-sm text-[#2D2D2D]">Итого</span>
                                <span className="text-sm text-[#2D2D2D]">{totalPrice.toLocaleString('ru-RU', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}₽</span>
                              </div>
                            </div>
                          </div>
                          ...
                          {order.package_image && (
                            <div className="mt-4">
                              <div className="text-sm text-[#858585] mb-2">Фото посылки</div>
                              <img
                              ...
                              {trackingLoading ? (
                                ...
                                <div className="flex flex-col">
                                  {tracking.tracking_history.map((item, index) => {
                                    const isLast = index === tracking.tracking_history.length - 1
                                    const isCompleted = true
                                    return (
                                      <div key={index} className="flex items-start gap-4">
                                        <div className="flex flex-col items-center shrink-0">
                                          {index > 0 && (
                                            <div className={`w-1.5 h-5 ${isCompleted ? 'bg-[#0077FE]' : 'bg-[#F4F2F3]'}`}></div>
                                          )}
                                          <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            {isCompleted ? (
                                              <>
                                                <circle cx="10" cy="10" r="9" fill="#0077FE" stroke="#0077FE" strokeWidth="1.5"/>
                                                <path d="M6 10L9 13L14 7" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                                              </>
                                            ) : (
                                              <circle cx="10" cy="10" r="9" stroke="#C8C7CC" strokeWidth="1.5"/>
                                            )}
                                          </svg>
                                          {!isLast && (
                                            <div className={`w-1.5 h-5 ${isCompleted ? 'bg-[#0077FE]' : 'bg-[#F4F2F3]'}`}></div>
                                        ...
                                        <div className="flex-1 pb-4">
                                          <p className="text-sm font-bold text-[#2D2D2D]">{item.status_name || item.status}</p>
                                          <p className="text-xs text-[#525252] mt-1">
                                            {formatDateTimeTracking(item.date_time)}
                                          </p>
                                        </div>
                                      </div>
                                    )
                          ...
                          <div className="mt-4 flex flex-col items-center py-2">
                            <div className="text-base font-bold text-[#2D2D2D] mb-2">Есть вопрос по доставке?</div>
                            <button className="text-sm text-[#0077FE]">Написать в поддержку</button>
                          </div>
                        </div>
                      )}
                    </div>
